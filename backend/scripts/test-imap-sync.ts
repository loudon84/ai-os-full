 ...
 *
 * Usage (from portal root): pnpm --filter @portal/server exec tsx ../../scripts/test-imap-sync.ts
 *
...
import { createDb, emailAccounts, users } from "@portal/db";
import { eq } from "drizzle-orm";

import { CredentialCryptoService } from "../src/services/email/credential-crypto.service.js";
import { ImapProvider } from "../src/services/email/providers/imap-provider.js";
import type { ReceiveConfig, FetchedMessage } from "../src/services/email/providers/mailbox-provider.js";
...
async function main() {
  ...
  // 1. Find user with email account (admin@portal.local or any user)
  let targetUser: { id: string; email: string } | null = null;

  // Try exact match first
  for (const email of ["admin@portal", "admin@portal.local"]) {
    const rows = await db
  ...
  // Fallback: find any user who has an email account
  if (!targetUser) {
    const rows = await db
      .select({ id: users.id, email: users.email })
      .from(users)
    ...
    for (const u of rows) {
      const accts = await db
        ...
        .from(emailAccounts)
        .where(eq(emailAccounts.userId, u.id))
        .limit(1);
      if (accts.length > 0) {
        targetUser = u;
        break;
  ...
  if (!targetUser) {
    console.error("❌ No user with email account found in database");
    process.exit(1);
  }
