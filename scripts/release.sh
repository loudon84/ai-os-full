...
Examples:
  ...
  ./scripts/release.sh canary --date 2026-05-10 --dry-run
  ./scripts/release.sh stable
  ./scripts/release.sh stable --date 2026-05-10 --dry-run

Notes:
  - Stable versions use YYYY.MDD.P format.
  - Canary releases publish YYYY.MDD.P-canary.N under npm dist-tag "canary".
  - Stable releases publish YYYY.MDD.P under npm dist-tag "latest".
  - The script rewrites versions temporarily and restores the working tree on exit.
EOF
}
...
if [ "$skip_verify" = false ]; then
  release_info ""
  release_info "==> Step 1/6: Verification gate..."
  cd "$REPO_ROOT"
  pnpm -r typecheck
  pnpm test
  pnpm build
  ...
  release_info ""
  release_info "==> Step 1/6: Verification gate skipped (--skip-verify)"
fi

release_info ""
release_info "==> Step 2/6: Building workspace artifacts..."
cd "$REPO_ROOT"
pnpm build
bash "$REPO_ROOT/scripts/prepare-frontend-dist.sh"
release_info "  ✓ Workspace build complete"

release_info ""
release_info "==> Step 3/6: Rewriting workspace versions..."
...
if [ "$dry_run" = true ]; then
  release_info "==> Step 4/6: Previewing publish payloads (--dry-run)..."
  ...
  release_info "==> Step 4/6: Publishing packages to npm..."
...
release_info ""
if [ "$dry_run" = true ]; then
  release_info "==> Step 5/6: Skipping npm verification in dry-run mode..."
else
  release_info "==> Step 5/6: Confirming npm package availability..."
...
release_info ""
if [ "$dry_run" = true ]; then
  release_info "==> Step 6/6: Dry run complete..."
else
  release_info "==> Step 6/6: Creating git tag..."
