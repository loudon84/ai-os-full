#!/usr/bin/env bash
#
# Kill all local Portal dev server processes.
#
# Usage:
#   scripts/kill-dev.sh        # kill all portal dev processes
...
while IFS= read -r line; do
  ...
  node_pids+=("$pid")
  node_lines+=("$line")
done < <(ps aux | grep -E '/portal/' | grep node | grep -v grep || true)

candidate_pidfiles=()
candidate_pidfiles+=(
  "$PORTAL_ROOT"/.portal/instances/*/db/postmaster.pid
  "$PORTAL_ROOT"/data/*/db/postmaster.pid
)
...
if [[ ${#node_pids[@]} -eq 0 && ${#pg_pids[@]} -eq 0 ]]; then
  echo "No Portal dev processes found."
  exit 0
fi

if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Found ${#node_pids[@]} Portal dev node process(es):"
  echo ""
...
if [[ ${#node_pids[@]} -gt 0 ]]; then
  echo "Sending SIGTERM to Portal node processes..."
